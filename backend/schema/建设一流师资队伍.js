...
//tch_moral_const 
 const data_3_1_1 = joi.array().items(
    joi.object().keys({
        ...
        yr:joi.number().integer().less(2025).greater(1950).required().error(new Error('获得年度填报错误！'))
    }
    )
)
//acleader_youthcore 
...
const data_3_2_2_0 = joi.array().items(
    joi.object().keys({
        team_name:joi.string().min(1).max(100).required().error(new Error('团队/人才姓名填报错误！')),
        ...
        yr:joi.number().integer().less(2029).greater(1950).required().error(new Error('获批年度填报错误！')),
...
const data_3_2_2_1 = joi.array().items(
   joi.object().keys({
       team_name:joi.string().min(1).max(100).required().error(new Error('团队/人才名称填报错误！')),
       level:joi.string().valid('国家级').required().error(new Error(new Error('层次填报错误！'))),
       honor_name:joi.string().min(1).max(100).required().error(new Error('荣誉称号填报错误！')),
       yr:joi.number().integer().less(2029).greater(1950).required().error(new Error('获批年度填报错误！')),
...
const data_3_2_3 = joi.array().items(
    joi.object().keys({
        yr:joi.number().integer().less(2029).greater(1950).required().error(new Error('学年度填报错误！')),
...
const data_3_2_4 = joi.array().items(
    joi.object().keys({
        ...
        in_postdoc_sum:joi.number().integer().required().error(new Error('国内毕业博士后总数填报错误！')),
        in_postdoc_newinc:joi.number().integer().required().error(new Error('国内毕业博士后今年新增填报错误！')),
        out_postdoc_sum:joi.number().integer().required().error(new Error('境外毕业博士后总数填报错误！')),
        out_postdoc_newinc:joi.number().integer().required().error(new Error('境外毕业博士后今年新增填报错误！')),
        univ_ra_sum:joi.number().integer().required().error(new Error('校聘科研助理人数总数填报错误！')),
        univ_ra_newinc:joi.number().integer().required().error(new Error('校聘科研助理人数今年新增填报错误！')),
        inst_ra_sum:joi.number().integer().required().error(new Error('院聘科研助理人数总数填报错误！')),
        inst_ra_newinc:joi.number().integer().required().error(new Error('院聘科研助理人数今年新增填报错误！')),
        task_ra_sum:joi.number().integer().required().error(new Error('课题聘科研助理人数总数填报错误！')),
        task_ra_newinc:joi.number().integer().required().error(new Error('课题聘科研助理人数今年新增填报错误！')),
    }
    )
)
...
const data_3_2_5 = joi.array().items(
    joi.object().keys({
        ...
        sum_full_ftch:joi.number().integer().required().error(new Error('数量(总数)填报错误！')),
        sum_high_title:joi.number().integer().required().error(new Error('正高级职称数量(总数)填报错误！')),
        lang_full_ftch:joi.number().integer().required().error(new Error('数量(语言类外籍专任教师数)填报错误！')),
        lang_high_title:joi.number().integer().required().error(new Error('正高级职称数量(语言类外籍专任教师)填报错误！')),
        prof_full_ftch:joi.number().integer().required().error(new Error('数量(专业类外籍专任教师)填报错误！')),
        prof_high_title:joi.number().integer().required().error(new Error('正高级职称数量(专业类外籍专任教师)填报错误！')),
...
const data_3_3_1 = joi.array().items(
    joi.object().keys({
        ...
        tenure:joi.string().pattern(/^(1[9][5-9][0-9]|2[0][0-2][0-9])-(1[9][5-9][0-9]|2[0][0-2][0-9])$/).required().error(new Error('任职期限填报错误！')),                   //
    }
    )
)
...
const data_3_3_2 = joi.array().items(
    joi.object().keys({
        ...
        yr:joi.number().integer().less(2029).greater(1950).error(new Error('填报年度填报错误！')),
    }
    )
)
...
const data_3_3_3 = joi.array().items(
    joi.object().keys({
        ...
        rpt_yr_mth:joi.string().pattern(/^(1[9][5-9][0-9]|2[0][0-2][0-9])-(0[1-9]|1[0-2])$/).error(new Error('报告年月填写错误！')),
...
const data_3_3_4 = joi.array().items(
    joi.object().keys({
        ...
        comp_yr_mth:joi.string().pattern(/^(1[9][5-9][0-9]|2[0][0-2][0-9])-(0[1-9]|1[0-2])$/).error(new Error('比赛年月填写错误！')),
        pos:joi.string().valid('主编','副主编','编委').required().error(new Error('担任职务填报错误！')),
    }
    )
)
