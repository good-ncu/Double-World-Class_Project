...
const data_3_2_2_0 = joi.array().items(
    joi.object().keys({
        talent_team_name:joi.string().min(1).max(100).required().error(new Error('团队/人才姓名填报错误！')),       // 改
...
const data_3_2_2_1 = joi.array().items(
   joi.object().keys({
       talent_team_name:joi.string().min(1).max(100).required().error(new Error('团队/人才名称填报错误！')),    // 改了
       honor_name:joi.string().min(1).max(100).required().error(new Error('荣誉称号填报错误！')),
...
const data_3_2_2_2 = joi.array().items(
   joi.object().keys({
       talent_team_name:joi.string().min(1).max(100).required().error(new Error('团队名称填报错误！')),    //改了
       honor_name:joi.string().min(1).max(100).required().error(new Error('荣誉称号填报错误！')),
       yr:joi.number().integer().less(2023).greater(2020).required().error(new Error('获批年度填报错误！')),
...
const data_3_2_3 = joi.array().items(
    joi.object().keys({
        ...
        full_tch_num:joi.number().integer().min(0).required().error(new Error('专任教师总数填报错误！')),    //  改了   都加了min(0)
        ...
        phd:joi.number().integer().min(0).required().error(new Error('博士数量填报错误！')),
        m_degree:joi.number().integer().min(0).required().error(new Error('硕士数量填报错误！')),
        b_degree:joi.number().integer().min(0).required().error(new Error('其他学历数量填报错误！')),
    }
    )
)
...
const data_3_2_4 = joi.array().items(
    joi.object().keys({
        ...
        task_ra_newinc:joi.number().integer().required().error(new Error('课题聘科研助理人数当年新增填报错误！')),
    }
    )
)
