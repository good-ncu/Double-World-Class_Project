 ...
 */
 ...
 const data_2_1_1 = joi.array().items(
    joi.object().keys({
        ...
        project_year:joi.number().integer().less(2025).greater(1950).required().error(new Error('项目年份填报错误！')),
    })
) 
const data_2_2_1_0 = joi.array().items(
    joi.object().keys({
        award_level:joi.string().min(1).max(100).required().error(new Error('奖项等级填报错误！')),
        award_type:joi.string().min(1).max(100).required().error(new Error('奖项类型填报错误！')),//
...
const data_2_2_2_1 = joi.array().items(
    joi.object().keys({
        ...
        sig:joi.string().min(1).max(13).required().error(new Error('署名情况填报错误！')),
...
const data_2_2_2_3 = joi.array().items(
    joi.object().keys({
        ...
        cour_type:joi.string().min(1).max(100).required().error(new Error('课程类别填报错误！')),
        appro_year:joi.number().integer().less(2025).greater(1950).required().error(new Error('获批年度填报错误！')),
        cour_level:joi.string().min(1).max(100).required().error(new Error('课程级别填报错误！')),
    }
    )
)
...
const data_2_3_2 = joi.array().items(
    joi.object().keys({
        stu_name:joi.string().min(1).max(13).required().error(new Error('学生姓名填报错误！')),
        grad_year:joi.number().integer().less(2025).greater(1950).required().error(new Error('毕业年份填报错误！')),
...
const data_2_4_1 = joi.array().items(
    joi.object().keys({
        b_enroll_year:joi.number().integer().less(2025).greater(1950).required().error(new Error('入学年度（攻学）填报错误！')),
        b_cur_num:joi.number().integer().min(0).required().error(new Error('在校学生数量（攻学）填报错误！')),
        m_enroll_year:joi.number().integer().less(205).greater(1950).required().error(new Error('入学年度（攻硕）填报错误！')),
        m_cur_num:joi.number().integer().min(0).required().error(new Error('在校学生数量（攻硕）填报错误！')),
        phd_enroll_year:joi.number().integer().less(2025).greater(1950).required().error(new Error('入学年度（攻博）填报错误！')),
