...
//tch_award 
 const data_4_1_1_0 = joi.array().items(
    joi.object().keys({
        tch_name:joi.string().min(1).max(1000).required().error(new Error('教师姓名填报错误！')),
        ...
        award_eval_org:joi.string().min(1).max(1000).required().error(new Error('评奖组织单位填报错误！')),
        award_eval_org_type:joi.string().min(1).max(1000).required().error(new Error('组织单位类型填报错误！')),
...
const data_4_1_1_1 = joi.array().items(
   joi.object().keys({
       ...
       grade:joi.string().valid('特等奖','一等奖','二等奖','三等奖').required().error(new Error('奖项等级填报错误！')),
       award_eval_org:joi.string().min(1).max(1000).required().error(new Error('评奖组织单位填报错误！')),
       award_eval_org_type:joi.string().min(1).max(1000).required().error(new Error('评奖组织单位类型填报错误！')),
...
const data_4_1_2 = joi.array().items(
    joi.object().keys({
        ...
        monograph:joi.string().min(1).max(1000).required().error(new Error('专著名称填报错误！')),
        tch_name:joi.string().min(1).max(1000).required().error(new Error('教师姓名填报错误！')),
        publisher:joi.string().min(1).max(1000).required().error(new Error('出版社填报错误！')),
        publish_date:joi.string().pattern(/^(2[0][2][1-9])-(0[1-9]|1[0-2])$/).error(new Error('出版时间填写错误！')),
    }
    )
)
...
const data_4_1_3_0 = joi.array().items(
    joi.object().keys({
        ...
        remarks: joi.string().max(1000).allow("").error(new Error('备注在200字以内！')),
...
const data_4_1_3_1 = joi.array().items(
    joi.object().keys({
        ...
        paper_title:joi.string().min(1).required().error(new Error('论文标题填报错误！')),
        paper_au:joi.string().min(1).max(1000).required().error(new Error('作者姓名填报错误！')),
        jour_name:joi.string().min(1).max(1000).required().error(new Error('发表期刊填报错误！')),
        yr_mth_volum:joi.string().min(1).required().error(new Error('发表年月及期刊卷（期）数填报错误！')),
        remarks: joi.string().max(1000).allow("").required().error(new Error('备注在200字以内！')),
...
const data_4_1_4 = joi.array().items(
   joi.object().keys({
       ...
       task: joi.string().max(1000).required().error(new Error('任务填写错误！')),
   }
   )
)
...
const data_4_2_2_0 = joi.array().items(
   joi.object().keys({
       yr:joi.number().integer().less(2021).greater(1950).required().error(new Error('填报年度填报错误！')),
       proj_name:joi.string().min(1).max(1000).required().error(new Error('项目名称填报错误！')),
...
const data_4_2_4 = joi.array().items(
   joi.object().keys({
       ...
       ac_influ:joi.string().min(1).max(1000).required().error(new Error('学术影响力填报错误！')),
...
const data_4_3_1 = joi.array().items(
   joi.object().keys({
       yr:joi.number().integer().less(2029).greater(2020).required().error(new Error('年度填报错误！')),
       proj_name:joi.string().min(1).max(1000).required().error(new Error('名称填报错误！')),
       ...
       parti_type:joi.string().min(1).max(1000).required().error(new Error('参与类型填报错误！')),              //
   }
   )
)
