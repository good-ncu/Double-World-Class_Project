...
//tch_award 
 const data_4_1_1_0 = joi.array().items(
    joi.object().keys({
        ...
        award_eval_org:joi.string().min(1).max(200).required().error(new Error('评奖组织单位填报错误！')),
        award_eval_org_type:joi.string().min(1).max(200).required().error(new Error('组织单位类型填报错误！')),
...
const data_4_1_1_1 = joi.array().items(
   joi.object().keys({
       ...
       grade:joi.string().valid('特等奖','一等奖','二等奖','三等奖').required().error(new Error('奖项等级填报错误！')),
       award_eval_org:joi.string().min(1).max(200).required().error(new Error('评奖组织单位填报错误！')),
       award_eval_org_type:joi.string().min(1).max(200).required().error(new Error('评奖组织单位类型填报错误！')),
...
const data_4_1_2 = joi.array().items(
    joi.object().keys({
        ...
        monograph:joi.string().min(1).max(200).required().error(new Error('专著名称填报错误！')),
        tch_name:joi.string().min(1).max(100).required().error(new Error('教师姓名填报错误！')),
...
const data_4_1_3_0 = joi.array().items(
    joi.object().keys({
        ...
        // yr_mth_volum:joi.string().error(new Error('发表年月及期刊卷（期）数填报错误！')),
