...
//tch_award 
 const data_4_1_1_0 = joi.array().items(
    joi.object().keys({
        ...
        level:joi.string().valid('国家级','省级').error(new Error(new Error('层次填报错误！'))),
    }
    )
)
...
const data_4_1_3_1 = joi.array().items(
    joi.object().keys({
        yr:joi.number().integer().less(2029).greater(1950).required().error(new Error('年度填报错误！')),
        quarter:joi.string().valid('第一季度','第二季度','第三季度','第四季度').error(new Error('季度填报错误！')),
        paper_title:joi.string().min(1).required().error(new Error('论文标题填报错误！')),
        paper_au:joi.string().min(1).max(200).required().error(new Error('作者姓名填报错误！')),
        jour_name:joi.string().min(1).max(100).required().error(new Error('发表期刊填报错误！')),
        volume_num:joi.number().integer().min(1).required().error(new Error('期刊卷（期）数填报错误！')),
...
const data_4_1_4 = joi.array().items(
   joi.object().keys({
       ...
       parti_date:joi.string().pattern(/^(1[9][5-9][0-9]|2[0][0-2][0-9])-(0[1-9]|1[0-2])$/).error(new Error('参与时间填写错误！')),
       task: joi.string().max(200).allow("").error(new Error('任务填写错误！')),
   }
   )
)

//scir_innova_platconst 
const data_4_2_1_0 = joi.array().items(
   joi.object().keys({
       yr:joi.number().integer().less(2029).greater(1950).required().error(new Error('填报年度填报错误！')),
       plat_name:joi.string().min(1).max(100).required().error(new Error('平台名称填报错误！')),
       palt_level:joi.string().valid('科技部国家级','其他国家级','省级').required().error(new Error(new Error('平台等级填报错误！'))),
       appro_time:joi.string().pattern(/^(1[9][5-9][0-9]|2[0][0-2][0-9])-(0[1-9]|1[0-2])$/).error(new Error('批准时间填写错误！')),
   }
   )
)
...
const data_4_2_2_0 = joi.array().items(
   joi.object().keys({
       ...
       proj_fromto_ymth:joi.string().pattern(/^(1[9][5-9][0-9]|2[0][0-2][0-9])-(0[1-9]|1[0-2])$/).error(new Error('项目起止年月填写错误！')),
...
const data_4_2_4 = joi.array().items(
   joi.object().keys({
       ...
       create_time:joi.string().pattern(/^(1[9][5-9][0-9]|2[0][0-2][0-9])-(0[1-9]|1[0-2])$/).error(new Error('创刊时间填写错误！')),
