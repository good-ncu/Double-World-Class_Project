...
const data_1_1_2 = joi.array().items(
    joi.object().keys({
        discipline_eval_turn: joi.number().integer().less(5).greater(3).required().error(new Error('学科评估轮次格式错误')),
        discipline_eval_result: joi.string().valid("A+","A","A-","B+","B","B-","C+","C","C-","Unknown").error(new Error('学科评估轮次格式错误')),
    })
)

const data_1_1_3 = joi.array().items(
    joi.object().keys({
        year:joi.number().integer().greater(2000).less(2023).required().error(new Error('年度填报错误！')),
...
const data_1_1_4 = joi.array().items(
    joi.object().keys({
        year:joi.number().integer().min(2000).max(2023).required().error(new Error('年度填报错误！')),
        ...
        other_receive_fund:joi.number().min(0).allow('').error(new Error('其他实际支出！')),
    })
)
