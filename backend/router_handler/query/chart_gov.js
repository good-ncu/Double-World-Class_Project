...
exports.gov_tjd_leaders = function(req,res) {
     ...
     */
     ...
     a.univ_name,
     a.discipline_name,
     COUNT(talent_team.id) AS rc_num 	--人才数量
   FROM
   ((
   SELECT
     univ_discipline.univ_code,
   ...
   GROUP BY
     ...
     a.univ_name,
     a.discipline_name
   ORDER BY rc_num DESC,univ_code,discipline_code ASC`
     client.query(sql, function (err, results) {
         if (err) {
             // 异常后调用callback并传入err
             res.send({
...
exports.gov_tjd_hold_big_project = function(req,res) {
     ...
     */
     ...
     SELECT
       ...
       a.univ_name,
       a.discipline_name,
       COUNT(host_sciproj.id) AS xm_num 	--xm数量
     FROM
     ((
     SELECT
       univ_discipline.univ_code,
     ...
     GROUP BY
       ...
       a.univ_name,
       a.discipline_name
     ORDER BY xm_num DESC,univ_code,discipline_code ASC`
     client.query(sql, function (err, results) {
         if (err) {
             // 异常后调用callback并传入err
             res.send({
