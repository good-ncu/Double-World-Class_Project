...
exports.auth2login = function (req, res) {
  ...
  const body = request('POST', 'https://basic.jxeduyun.com/sso-oauth/getOpenId', {
    headers: {
      //   'Content-Type': 'application/json'
      "content-type": "application/x-www-form-urlencoded"
    },
    body: 'appkey=' + config.appId + "&" + 'appsecret=' + config.appSecret + "&" + "accessToken=" + accessToken
  });

  var zhjyUserId = JSON.parse(body.getBody('utf8')).data

  console.log("获取江西智慧教育平台用户信息成功，zhjyUserId：" + zhjyUserId)

  const findUserIdSql = `select * from zhjy_user where zhjy_user_id = '${zhjyUserId}' and del_flag = 0`
  //根据 zhjyUserId 查询 userId
  client.query(findUserIdSql, (err, findUserIdResults) => {
    if (err) {
      console.log(err.message);
      return res.cc(err)
    }
    console.log('登录信息：')
    console.log('zhjyUserId：' + zhjyUserId);
    // 检查有无该用户
    if (findUserIdResults.rows.length < 1) {
      return res.cc('无该用户，登录失败')
    }

    var userId = findUserIdResults.rows[0].user_id
    //根据 userId 查询 用户信息
    const findUserSql = `select * from user_info where id = '${userId}'`
    // 执行SQL
    client.query(findUserSql, (err, findUserResults) => {
      if (err) {
        console.log(err.message);
        return res.cc(err)
      }
      if (findUserResults.rows.length < 1) {
        return res.cc('无该用户，登录失败')
      }
      const user = { ...findUserResults.rows[0], passwd: '' }

      loginByUser(user, res)
    })
  })
}
